...
const Anecdote = ({ anecdotes }) => {
  console.log("from Anecdote ", anecdotes)
  return (
    <div>
      <h2>{anecdotes.content} by {anecdotes.author}</h2>
      <p>Has {anecdotes.votes} votes</p>
      <p>For more information see <a href={anecdotes.info}>{anecdotes.info}</a></p>
    </div>
  );
};
...
const App = () => {
  ...
  return (
    ...
    <h1>Software anecdotes</h1>
          ...
          <Menu />
          <div style={{backgroundColor: "lightblue"}}>{notification ? notification : null}</div>
          <Route exact path="/" render={() => <AnecdoteList anecdotes={anecdotes} />} />
          <Route exact path="/anecdote/:id" render={({ match }) => <Anecdote anecdotes={anecdoteById(match.params.id)} />} />
          <Route exact path="/about" render={() => <About />} />
          <Route exact path="/create" render={() => <WithRouterCreate addNew={addNew}/>} />
